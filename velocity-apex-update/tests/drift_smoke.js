...
const flat={roadWidth:1000,barrierMargin:200,theme:{drag:1},samples:[{tx:1,ty:0,nx:0,ny:1}],nearest(x,y){return{index:0,progress:(((x%10000)+10000)%10000)/10000,signed:y,tx:1,ty:0,nx:0,ny:1,x,y:0,curve:0};}};
...
assert.ok(peakSlip>=12&&peakSlip<65,`handbrake drift slip out of useful range: ${peakSlip.toFixed(1)}°`);
const speedAfterInitiation=car.speed;assert.ok(speedAfterInitiation>240,`drift should preserve speed instead of auto-braking: ${speedAfterInitiation.toFixed(1)}`);
...
// Every difficulty must be able to finish each long drift route using the real drift physics.
const aiReport=[];
for(const cfg of Object.values(R.DRIFT_TRACKS))for(const difficulty of DIFF){
  const track=new R.Track(cfg),bot=new R.Car({id:1,...R.RACE_PHYSICS});bot.place(track,0,0);bot.ai=new R.AIController(bot,1,difficulty);bot.ai.driftMode=true;
  let time=0,barrierImpacts=0,aiPeakSlip=0,sawDriftCommand=false;
  while(bot.laps<1&&time<150){const out=bot.ai.think(1/60,track,[bot]);sawDriftCommand=sawDriftCommand||out.drift===true;for(let q=0;q<2;q++){const r=bot.update(DT,out,track);aiPeakSlip=Math.max(aiPeakSlip,slipDeg(bot));if(r.impact>.08)barrierImpacts++;time+=DT;}}
  assert.ok(bot.laps>=1,`${cfg.id}/${difficulty}: AI failed to complete drift route`);
  assert.ok(sawDriftCommand,`${cfg.id}/${difficulty}: AI never enabled drift physics`);
  assert.ok(aiPeakSlip>=7,`${cfg.id}/${difficulty}: AI did not produce a meaningful slip angle (${aiPeakSlip.toFixed(1)}°)`);
  assert.ok(barrierImpacts<=40,`${cfg.id}/${difficulty}: too many barrier impacts (${barrierImpacts})`);
  aiReport.push({track:cfg.id,difficulty,time:+time.toFixed(1),barrierImpacts,peakSlipDeg:+aiPeakSlip.toFixed(1)});
}

// Drift payout is tied to completion context but score contribution is capped.
const settings={bots:7,difficulty:'medium'};
const base=R.driftReward(settings,8,0,10400),winner=R.driftReward(settings,1,0,10400),scored=R.driftReward(settings,1,20000,10400),huge=R.driftReward(settings,1,99999999,10400);
...
assert.ok(R.driftReward({...settings,difficulty:'extreme'},1,20000,11800)>scored,'higher difficulty/longer route should pay more');
...
assert.ok(game.includes("forward>42")&&game.includes("meters>.025")&&game.includes("progressMeters"),'drift scoring must require forward track progress');
assert.ok(game.includes("playerControl.drift=localMode==='drift'"),'normal and drift physics must remain mode-separated');
assert.ok(game.includes('modeSelectReadyAt=performance.now()+420'),'play must guard against iOS ghost taps skipping mode selection');
assert.ok(game.includes('claimDriftCreditsOnce'),'drift rewards must have persistent duplicate-claim protection');

console.log(JSON.stringify({tracks:trackReport,physics:{peakSlipDeg:+peakSlip.toFixed(1),speedAfterInitiation:+speedAfterInitiation.toFixed(1),recoveredSlipDeg:+recoveredSlip.toFixed(1)},ai:aiReport,payout:{base,winner,scored,capped:huge}},null,2));
