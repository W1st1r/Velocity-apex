(function(){
  ...
  const remaining=expiresAt=>{const ms=Math.max(0,Number(expiresAt||0)-Date.now()),m=Math.ceil(ms/60000);if(m<60)return m+' мин';const h=Math.ceil(m/60);return h<48?h+' ч':Math.ceil(h/24)+' д';};

  class RootConsole{
    constructor(opts){
      this.save=opts.save;this.onChange=opts.onChange||(()=>{});this.onBack=opts.onBack||(()=>{});this.onView=opts.onView||(()=>{});
      this.authorized=false;this.confirmAction=null;this.lastMessage='';this.authTimer=0;this.tab='money';this.remoteDetail=null;this.remoteResourceTab='car';this.remoteContext='accounts';this.searchTimer=0;
    ...
    async searchAccounts(blocked){
      if(!this.authorized)return;const input=blocked?this.blockedSearch:this.accountSearch,list=blocked?this.blockedList:this.accountList,q=input.value.trim().toLowerCase();list.innerHTML='<div class="root-loading">ЗАГРУЗКА…</div>';
    ...
    renderAccountList(list,accounts,blocked){if(!accounts.length){list.innerHTML='<div class="root-empty">НИЧЕГО НЕ НАЙДЕНО</div>';return;}list.innerHTML=accounts.map(a=>`<button class="root-account-row" type="button" data-account-id="${esc(a.id)}"><span class="root-account-avatar">${esc((a.displayName||a.username||'VA').slice(0,2).toUpperCase())}</span><span class="root-account-copy"><strong>${esc(a.displayName||a.username)}</strong><em>@${esc(a.username)}</em><small>${a.lastLoginAt?'Последний вход: '+esc(when(a.lastLoginAt)):'Ещё не входил'}</small></span>${a.ban?`<span class="root-ban-chip">BAN · ${esc(remaining(a.ban.expiresAt))}</span>`:'<span class="root-open-chip">ОТКРЫТЬ ›</span>'}</button>`).join('');}
    ...
    async openAccount(id,blocked){
      const detail=blocked?this.blockedDetail:this.accountDetail;detail.classList.remove('hidden');detail.innerHTML='<div class="root-loading">ЗАГРУЗКА ПРОФИЛЯ…</div>';this.remoteContext=blocked?'blocked':'accounts';this.remoteResourceTab='car';
      try{const data=await this.api('/api/admin/account/'+encodeURIComponent(id));this.remoteDetail=data;this.renderRemoteDetail(detail,data,blocked);detail.scrollIntoView?.({block:'nearest'});}catch(e){detail.innerHTML=`<div class="root-empty">${esc(this.adminError(e.code))}</div>`;}
    }
    renderRemoteDetail(container,data,blocked){
      ...
      const resourceList=this.remoteResourceList(data,this.remoteResourceTab);
      container.innerHTML=`<div class="root-admin-profile"><span class="root-account-avatar large">${esc((u.displayName||u.username).slice(0,2).toUpperCase())}</span><div><small>USER ID · ${esc(u.id.slice(0,8))}</small><strong>${esc(u.displayName)}</strong><em>@${esc(u.username)}</em></div><div class="root-admin-profile-meta"><span>${fmt(s.credits)} CR</span><small>REV ${Number(data.revision)||0}</small></div></div>
        ${ban?`<div class="root-ban-active"><strong>АКТИВНАЯ БЛОКИРОВКА · ${esc(remaining(ban.expiresAt))}</strong><p>${esc(ban.reason)}</p><small>До ${esc(when(ban.expiresAt))}</small><button type="button" data-admin-act="unban">РАЗБЛОКИРОВАТЬ</button></div>`:''}
        <div class="root-admin-section"><div class="root-module-title"><span>ДЕНЬГИ</span><small>REMOTE SAVE</small></div><div class="root-admin-credit-row"><input class="root-input root-admin-credit-input" type="text" inputmode="numeric" placeholder="Сумма CR"><button type="button" data-admin-act="credit-add">ДОБАВИТЬ</button><button type="button" class="danger" data-admin-act="credit-take">ЗАБРАТЬ</button></div></div>
        <div class="root-admin-section"><div class="root-module-title"><span>РЕСУРСЫ</span><small>GRANT / REVOKE</small></div><div class="root-admin-resource-tabs"><button class="${this.remoteResourceTab==='car'?'active':''}" type="button" data-admin-resource-tab="car">МАШИНЫ</button><button class="${this.remoteResourceTab==='effect'?'active':''}" type="button" data-admin-resource-tab="effect">ЭФФЕКТЫ</button><button class="${this.remoteResourceTab==='case'?'active':''}" type="button" data-admin-resource-tab="case">КЕЙСЫ</button></div><div class="root-admin-resource-list">${resourceList}</div></div>
        <div class="root-admin-section root-ban-editor ${protectedAdmin?'hidden':''}"><div class="root-module-title"><span>БЛОКИРОВКА</span><small>ACCOUNT ACCESS</small></div><textarea class="root-admin-ban-reason" maxlength="240" placeholder="Причина блокировки"></textarea><div class="root-admin-ban-row"><input class="root-input root-admin-ban-duration" type="number" min="1" max="365" value="1" inputmode="numeric"><select class="root-admin-ban-unit"><option value="hour">ЧАСОВ</option><option value="day" selected>ДНЕЙ</option></select><button type="button" class="danger" data-admin-act="ban">ЗАБЛОКИРОВАТЬ</button></div></div>
        ${protectedAdmin?'<div class="root-protected-note">Администраторский аккаунт защищён от блокировки.</div>':''}`;
    }
    ...
    async _remoteAction(e,blocked){
      const tab=e.target.closest('[data-admin-resource-tab]');if(tab){this.remoteResourceTab=tab.dataset.adminResourceTab;this.renderRemoteDetail(blocked?this.blockedDetail:this.accountDetail,this.remoteDetail,blocked);return;}
      const btn=e.target.closest('[data-admin-act]');if(!btn||!this.remoteDetail?.user)return;const id=this.remoteDetail.user.id,container=blocked?this.blockedDetail:this.accountDetail,action=btn.dataset.adminAct;btn.disabled=true;
      try{
        let data;
