(function(){
  ...
  class RootConsole{
    constructor(opts){
      ...
      this.authorized=false;this.confirmAction=null;this.lastMessage='';this.authTimer=0;this.tab='money';this.remoteDetail=null;this.remoteDetailTab='overview';this.remoteResourceTab='car';this.remoteContext='accounts';this.searchTimer=0;this.promoRows=[];this.promoEditingId='';
      ...
      this.tabs=$('rootTabs');this.accountSearch=$('rootAccountSearch');this.accountList=$('rootAccountList');this.accountDetail=$('rootAccountDetail');this.blockedSearch=$('rootBlockedSearch');this.blockedList=$('rootBlockedList');this.blockedDetail=$('rootBlockedDetail');this.promoList=$('rootPromoList');this.promoCode=$('rootPromoCode');this.promoTitle=$('rootPromoTitle');this.promoCredits=$('rootPromoCredits');this.promoMaxUses=$('rootPromoMaxUses');this.promoCar=$('rootPromoCar');this.promoEffect=$('rootPromoEffect');this.promoCase=$('rootPromoCase');this.promoCaseQty=$('rootPromoCaseQty');this.promoDuration=$('rootPromoDuration');this.promoDurationUnit=$('rootPromoDurationUnit');this.promoEnabled=$('rootPromoEnabled');this.promoEditorTitle=$('rootPromoEditorTitle');
      this.confirm=$('rootConfirm');this.confirmTitle=$('rootConfirmTitle');this.confirmText=$('rootConfirmText');this._bind();this.preparePromoOptions();
    }
    ...
    _bind(){
      ...
      $('rootPromoSave')?.addEventListener('click',()=>this.savePromocode());$('rootPromoReset')?.addEventListener('click',()=>this.resetPromoForm());this.promoList?.addEventListener('click',e=>this.promoListAction(e));this.promoCode?.addEventListener('input',()=>{this.promoCode.value=this.promoCode.value.toUpperCase().replace(/[^A-Z0-9_-]/g,'').slice(0,32);});
    }
    ...
    switchTab(tab){
      const allowed=['money','cars','effects','cases','promocodes','accounts','blocked'];if(!allowed.includes(tab))tab='money';this.tab=tab;
      ...
      if(tab==='accounts')this.searchAccounts(false);if(tab==='blocked')this.searchAccounts(true);if(tab==='promocodes')this.loadPromocodes();this.renderLocal();
    }
    ...
    preparePromoOptions(){
      if(!this.promoCar||this.promoCar.dataset.ready)return;this.promoCar.dataset.ready='1';
      this.promoCar.innerHTML='<option value="">БЕЗ МАШИНЫ</option>'+Object.entries(R.LIVERIES||{}).map(([id,x])=>`<option value="${esc(id)}">${esc(x.name||id)}</option>`).join('');
      this.promoEffect.innerHTML='<option value="">БЕЗ ЭФФЕКТА</option>'+Object.entries(R.EFFECTS||{}).map(([id,x])=>`<option value="${esc(id)}">${esc(x.name||id)}</option>`).join('');
      this.promoCase.innerHTML='<option value="">БЕЗ КЕЙСА</option>'+(R.CASE_ORDER||[]).map(id=>`<option value="${esc(id)}">${esc(R.CASES?.[id]?.name||id)}</option>`).join('');
    }
    resetPromoForm(){this.promoEditingId='';if(this.promoEditorTitle)this.promoEditorTitle.textContent='НОВЫЙ ПРОМОКОД';this.promoCode.value='';this.promoTitle.value='';this.promoCredits.value='0';this.promoMaxUses.value='0';this.promoCar.value='';this.promoEffect.value='';this.promoCase.value='';this.promoCaseQty.value='1';this.promoDuration.value='7';this.promoDurationUnit.value='day';this.promoEnabled.checked=true;}
    promoRewardText(r={}){const a=[];if(r.credits)a.push(fmt(r.credits)+' CR');for(const id of r.cars||[])a.push(R.LIVERIES?.[id]?.name||id);for(const id of r.effects||[])a.push(R.EFFECTS?.[id]?.name||id);for(const [id,q] of Object.entries(r.cases||{}))a.push(`${R.CASES?.[id]?.name||id} ×${q}`);return a.join(' · ')||'НЕТ НАГРАДЫ';}
    promoStatus(p){if(!p.enabled)return 'ОТКЛЮЧЁН';if(p.startsAt>Date.now())return 'ОЖИДАЕТ';if(p.expiresAt&&p.expiresAt<=Date.now())return 'ИСТЁК';if(p.maxUses&&p.usesCount>=p.maxUses)return 'ЛИМИТ';return 'АКТИВЕН';}
    async loadPromocodes(){if(!this.authorized||!this.promoList)return;this.promoList.innerHTML='<div class="root-loading">ЗАГРУЗКА…</div>';try{const data=await this.api('/api/admin/promocodes');this.promoRows=data.promocodes||[];this.renderPromocodes();}catch(e){this.promoList.innerHTML=`<div class="root-empty">${esc(this.adminError(e.code))}</div>`;if(e.code==='ADMIN_LOCKED')this.showAuth();}}
    renderPromocodes(){if(!this.promoList)return;if(!this.promoRows.length){this.promoList.innerHTML='<div class="root-empty">ПРОМОКОДОВ ПОКА НЕТ</div>';return;}this.promoList.innerHTML=this.promoRows.map(p=>{const status=this.promoStatus(p),active=status==='АКТИВЕН',limit=p.maxUses?`${p.usesCount} / ${p.maxUses}`:`${p.usesCount} / ∞`,time=p.expiresAt?`до ${when(p.expiresAt)}`:'без срока';return `<article class="root-promo-row ${active?'active':''}"><div class="root-promo-code"><small>${esc(status)}</small><strong>${esc(p.code)}</strong><span>${esc(p.title||'Без названия')}</span></div><div class="root-promo-reward"><small>НАГРАДА</small><strong>${esc(this.promoRewardText(p.reward))}</strong><span>${esc(limit)} активаций · ${esc(time)}</span></div><div class="root-promo-row-actions"><button type="button" data-promo-edit="${esc(p.id)}">ИЗМЕНИТЬ</button><button type="button" class="${p.enabled?'danger':''}" data-promo-toggle="${esc(p.id)}" data-enabled="${p.enabled?'0':'1'}">${p.enabled?'ОТКЛЮЧИТЬ':'ВКЛЮЧИТЬ'}</button></div></article>`;}).join('');}
    editPromocode(id){const p=this.promoRows.find(x=>x.id===id);if(!p)return;this.promoEditingId=p.id;this.promoEditorTitle.textContent='РЕДАКТИРОВАНИЕ · '+p.code;this.promoCode.value=p.code;this.promoTitle.value=p.title||'';this.promoCredits.value=String(p.reward?.credits||0);this.promoMaxUses.value=String(p.maxUses||0);this.promoCar.value=p.reward?.cars?.[0]||'';this.promoEffect.value=p.reward?.effects?.[0]||'';const cases=Object.entries(p.reward?.cases||{});this.promoCase.value=cases[0]?.[0]||'';this.promoCaseQty.value=String(cases[0]?.[1]||1);if(p.expiresAt){const ms=Math.max(3600000,p.expiresAt-p.startsAt);if(ms%86400000===0){this.promoDurationUnit.value='day';this.promoDuration.value=String(Math.min(365,Math.round(ms/86400000)));}else{this.promoDurationUnit.value='hour';this.promoDuration.value=String(Math.min(8760,Math.round(ms/3600000)));}}else{this.promoDuration.value='0';this.promoDurationUnit.value='day';}this.promoEnabled.checked=!!p.enabled;this.promoCode.focus();}
    async promoListAction(e){const edit=e.target.closest('[data-promo-edit]');if(edit){this.editPromocode(edit.dataset.promoEdit);return;}const toggle=e.target.closest('[data-promo-toggle]');if(!toggle)return;toggle.disabled=true;try{await this.api(`/api/admin/promocode/${toggle.dataset.promoToggle}/toggle`,{method:'POST',body:JSON.stringify({enabled:toggle.dataset.enabled==='1'})});this.message(toggle.dataset.enabled==='1'?'PROMOCODE ENABLED':'PROMOCODE DISABLED');await this.loadPromocodes();}catch(err){this.message(this.adminError(err.code),'bad');}finally{toggle.disabled=false;}}
    async savePromocode(){
      const code=this.promoCode.value.trim().toUpperCase(),credits=Math.max(0,Math.floor(Number(this.promoCredits.value)||0)),maxUses=Math.max(0,Math.floor(Number(this.promoMaxUses.value)||0)),caseId=this.promoCase.value,caseQty=Math.max(1,Math.min(99,Math.floor(Number(this.promoCaseQty.value)||1))),duration=Math.max(0,Math.floor(Number(this.promoDuration.value)||0)),unit=this.promoDurationUnit.value,unitMs=unit==='hour'?3600000:86400000,startsAt=Date.now(),expiresAt=duration?startsAt+duration*unitMs:0;
      const reward={credits,cars:this.promoCar.value?[this.promoCar.value]:[],effects:this.promoEffect.value?[this.promoEffect.value]:[],cases:caseId?{[caseId]:caseQty}:{}};const saveBtn=$('rootPromoSave');saveBtn.disabled=true;
      try{const data=await this.api('/api/admin/promocodes',{method:'POST',body:JSON.stringify({id:this.promoEditingId||undefined,code,title:this.promoTitle.value.trim(),reward,maxUses,startsAt,expiresAt,enabled:this.promoEnabled.checked})});this.message(`PROMOCODE ${data.promocode.code} SAVED`);this.resetPromoForm();await this.loadPromocodes();}
      catch(err){this.message(this.adminError(err.code),'bad');if(err.code==='ADMIN_LOCKED')this.showAuth();}finally{saveBtn.disabled=false;}
    }
    adminError(code){return ({ADMIN_LOCKED:'ПАНЕЛЬ ЗАБЛОКИРОВАНА · ВВЕДИТЕ ROOT ПАРОЛЬ',ADMIN_REQUIRED:'НЕТ ПРАВ АДМИНИСТРАТОРА',USER_NOT_FOUND:'АККАУНТ НЕ НАЙДЕН',INVALID_AMOUNT:'НЕВЕРНАЯ СУММА',INVALID_BAN_DURATION:'НЕВЕРНЫЙ СРОК БЛОКИРОВКИ',INVALID_BAN_REASON:'УКАЖИТЕ ПРИЧИНУ',ADMIN_ACCOUNT_PROTECTED:'АККАУНТ АДМИНИСТРАТОРА ЗАЩИЩЁН',SYSTEM_RESOURCE:'СИСТЕМНЫЙ РЕСУРС НЕЛЬЗЯ ЗАБРАТЬ',PROMO_INVALID:'НЕВЕРНЫЙ КОД · 3–32: A-Z, 0-9, _ И -',PROMO_REWARD_INVALID:'ДОБАВЬТЕ ХОТЯ БЫ ОДНУ НАГРАДУ',PROMO_LIMIT_INVALID:'НЕВЕРНЫЙ ЛИМИТ АКТИВАЦИЙ',PROMO_LIMIT_BELOW_USED:'ЛИМИТ МЕНЬШЕ УЖЕ ИСПОЛЬЗОВАННЫХ АКТИВАЦИЙ',PROMO_TIME_INVALID:'НЕВЕРНЫЙ СРОК ДЕЙСТВИЯ',PROMO_CODE_TAKEN:'ТАКОЙ ПРОМОКОД УЖЕ СУЩЕСТВУЕТ',PROMO_NOT_FOUND:'ПРОМОКОД НЕ НАЙДЕН'}[code]||'ОШИБКА АДМИН-ПАНЕЛИ');}
  }
