/* 42-car catalog, per-car geometry, asset, save, purchase and offline-bot regression test. */
...
const expectedPrices={
  ...
  'ferrari-laferrari':245000,'bugatti-chiron-super-sport':300000,'bugatti-divo':275000,'koenigsegg-jesko':310000,'pagani-huayra-bc':255000,'lamborghini-veneno':285000,'mercedes-amg-one':265000,'aston-martin-valkyrie-mary':280000,'apollo-evo':318000
};
const ids=Object.keys(expectedPrices);assert.equal(ids.length,42);assert.equal(R.REAL_CAR_IDS.length,42);assert.equal(new Set(R.REAL_CAR_IDS).size,42,'IDs must be unique');
...
assert.deepEqual(counts,{basic:6,sport:7,premium:6,rare:7,legendary:7,lux:9});assert.equal(new Set(prices).size,42,'real-car prices should be unique');
...
assert.equal(old.saveVersion,6);assert.equal(old.credits,777);assert.equal(old.selectedLivery,'porsche-911');assert.ok(old.ownedLiveries.includes('crimsonVelocity'));assert.equal(old.bestScore,99);assert.equal(old.bestLap,1234);
...
assert.ok(R.getCatalogEntries(s,'garage','livery','lux').some(([id])=>id==='aston-martin-valkyrie-mary'),'Mary missing from Garage');assert.equal(R.getCatalogEntries(s,'shop','livery','all').length,42,'Shop must show 42 real cars only');
...
console.log('car_catalog_smoke: OK (42 cars + Apollo EVO + Mary + geometry + OBB + saves + offline bot loadout)');
