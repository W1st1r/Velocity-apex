...
assert.ok(validSettings({trackId:'apexCircuit',laps:5,maxPlayers:8,collisions:false}));assert.ok(validSettings({trackId:'sierraFlow',laps:1,maxPlayers:8,collisions:false,drift:true}));assert.equal(validSettings({trackId:'sierraFlow',laps:1,maxPlayers:8,collisions:false,drift:false}),false);assert.equal(validSettings({trackId:'apexCircuit',laps:1,maxPlayers:8,collisions:false,drift:true}),false);assert.ok(validSettings({trackId:'auroraGrandLoop',laps:5,maxPlayers:8,collisions:false,wager:500}));assert.equal(validSettings({trackId:'bad',laps:5,maxPlayers:8,collisions:false}),false);assert.equal(validSettings({trackId:'apexCircuit',laps:5,maxPlayers:8,collisions:false,wager:333}),false);
assert.ok(WAGER_OPTIONS.includes(500)&&validWager(500)&&!validWager(333));
assert.deepEqual(normalizeSettings({trackId:'bad',laps:99,maxPlayers:22,collisions:true,wager:333}),{trackId:'apexCircuit',laps:5,maxPlayers:8,collisions:true,wager:0,drift:false});
assert.deepEqual(normalizeSettings({trackId:'midnightSwitchbacks',laps:5,maxPlayers:4,collisions:false,wager:500,drift:true}),{trackId:'midnightSwitchbacks',laps:1,maxPlayers:4,collisions:false,wager:500,drift:true});
...
assert.ok(validPlayerState(snap));assert.ok(validPlayerState({...snap,driftScore:12345}));assert.equal(validPlayerState({...snap,driftScore:-1}),false);assert.ok(validPlayerState({...snap,speed:510,vx:510}));assert.equal(validPlayerState({...snap,x:Infinity}),false);assert.equal(validPlayerState({...snap,speed:651}),false);assert.equal(validPlayerState({...snap,speed:900}),false);
