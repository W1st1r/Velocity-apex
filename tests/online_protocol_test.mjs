...
assert.ok(validSettings({trackId:'apexCircuit',laps:5,maxPlayers:8,collisions:false}));assert.equal(validSettings({trackId:'bad',laps:5,maxPlayers:8,collisions:false}),false);
...
assert.ok(validPlayerState(snap));assert.equal(validPlayerState({...snap,x:Infinity}),false);assert.equal(validPlayerState({...snap,speed:900}),false);
