...
const env={ROOMS:{idFromName(x){return x},get(){return room}},DB:{async batch(){return []},prepare(sql){return {bind(){return this},async run(){return {}},async first(){if(sql.includes('FROM crews'))return null;if(sql.includes('owner_settings'))return null;if(sql.includes('FROM sessions'))return identity?{token_hash:'test',user_id:'account',username:identity,display_name:'FAKE OWNER',created_at:1,expires_at:expired?1:Date.now()+100000,last_seen_at:Date.now()}:null;if(sql.includes('FROM account_bans'))return banned?{id:'ban',expires_at:Date.now()+100000}:null;throw Error(sql)}}}}};
