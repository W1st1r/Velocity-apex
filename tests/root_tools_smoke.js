...
assert(css.includes('#app *:not(input)')&&css.includes('-webkit-touch-callout:none')&&css.includes('#app input,#app textarea'),'central iOS interaction hardening missing');
assert(game.includes("['selectstart','dragstart','dblclick']")&&game.includes('performance.now()-lastHandled<700'),'app gesture guard or bindTap synthetic-click guard missing');
