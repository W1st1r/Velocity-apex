...
await recordResult(env,'drag1',ids[1],'drag',true,5300);await recordResult(env,'drag1',ids[1],'drag',true,5300);const profile=await call('/api/owner/profile/'+ids[1]);assert.equal(profile.stats.races,1);assert.equal(profile.stats.wins,1);assert.equal(profile.stats.bestDrag,5300);
