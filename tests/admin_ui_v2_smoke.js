...
for(const token of ['РЕСУРСЫ И МАШИНЫ','УПРАВЛЕНИЕ ИГРОКАМИ','root-account-workspace','ВЫБЕРИТЕ АККАУНТ'])assert(html.includes(token),'missing structured admin UI: '+token);
for(const token of ['ДАТА РЕГИСТРАЦИИ','ПОСЛЕДНИЙ ЗАХОД','data-admin-detail-tab="overview"','data-admin-detail-tab="money"','data-admin-detail-tab="resources"','data-admin-detail-tab="ban"'])assert(js.includes(token),'missing account detail information/navigation: '+token);
