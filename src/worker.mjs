import {PROTOCOL_VERSION,generateRoomCode,normalizeRoomCode,validRoomCode,sanitizeNickname,validNickname,validSettings,normalizeSettings,validPlayerState,parseClientMessage,PLAYER_STATE_RATE_MS,RECONNECT_GRACE_MS,EMPTY_ROOM_TTL_MS,ROOM_TTL_MS,TRACK_IDS,LAP_OPTIONS} from './protocol.mjs';
...
export class Room {
  ...
  async load(){
    ...
    if(this.room){this.room.players=this.room.players||[];this.room.finishOrder=this.room.finishOrder||[];}
  }
  ...
  publicRoom(){
    const r=this.room;return {protocol:PROTOCOL_VERSION,code:r.code,status:r.status,settings:r.settings,hostId:r.hostId,raceId:r.raceId||null,raceStartAt:r.raceStartAt||0,gridOrder:r.gridOrder||[],finishOrder:r.finishOrder||[],players:r.players.map(p=>({id:p.id,name:p.name,ready:!!p.ready,connected:!!p.connected,host:p.id===r.hostId,liveryId:p.liveryId,effectId:p.effectId,finishPlace:p.finishPlace||0,finishTime:p.finishTime||0,state:p.state?{laps:p.state.laps,progress:p.state.progress,finished:!!p.state.finished}:null}))};
  }
  ...
  async fetch(request){
    ...
    if(url.pathname==='/create'&&request.method==='POST'){
      ...
      const b=await request.json(),created=now(),id=playerId(),sessionToken=token();
      this.room={code:b.code,createdAt:created,updatedAt:created,emptySince:created,status:'lobby',settings:normalizeSettings(b.settings),hostId:id,raceId:null,raceStartAt:0,gridOrder:[],finishOrder:[],players:[{id,name:sanitizeNickname(b.name),token:sessionToken,ready:false,connected:false,disconnectedUntil:created+RECONNECT_GRACE_MS,liveryId:b.loadout?.liveryId||'apexLime',effectId:b.loadout?.effectId||'standard',lastSeq:-1,lastStateAt:0,state:null,serverLaps:0,lastLapAt:0,finishPlace:0,finishTime:0}]};
    ...
    if(url.pathname==='/join'&&request.method==='POST'){
      ...
      const id=playerId(),sessionToken=token(),t=now();this.room.players.push({id,name:sanitizeNickname(b.name),token:sessionToken,ready:false,connected:false,disconnectedUntil:t+RECONNECT_GRACE_MS,liveryId:safeLoadout(b.loadout).liveryId,effectId:safeLoadout(b.loadout).effectId,lastSeq:-1,lastStateAt:0,state:null,serverLaps:0,lastLapAt:0,finishPlace:0,finishTime:0});
    ...
    if(url.pathname==='/ws'||url.pathname.endsWith('/ws')){
      ...
      if(!this.room)return json({error:'ROOM_NOT_FOUND'},404);
      const id=url.searchParams.get('playerId')||'',sessionToken=url.searchParams.get('token')||'';const p=this.room.players.find(x=>x.id===id&&x.token===sessionToken);
  ...
  async webSocketMessage(ws,message){
    ...
    if(m.type==='ready'){
      if(this.room.status!=='lobby'||typeof m.ready!=='boolean')return;p.ready=m.ready;await this.save();this.broadcast({type:'room_state',v:PROTOCOL_VERSION,serverTime:now(),room:this.publicRoom()});return;
    }
    if(m.type==='settings_update'){
      ...
      if(m.settings.maxPlayers<this.room.players.filter(x=>x.connected).length)return this.error(ws,'ROOM_TOO_LARGE','Сначала уменьшите число игроков');
      this.room.settings=normalizeSettings(m.settings);await this.save();this.broadcast({type:'room_state',v:PROTOCOL_VERSION,serverTime:now(),room:this.publicRoom()});return;
    }
    if(m.type==='start_race'){
      ...
      for(const x of this.room.players){x.finishPlace=0;x.finishTime=0;x.state=null;x.lastSeq=-1;x.lastStateAt=0;x.serverLaps=0;x.lastLapAt=this.room.raceStartAt;}
    ...
    if(m.type==='race_finish'){
      ...
      p.finishPlace=this.room.finishOrder.length+1;p.finishTime=Math.max(0,now()-this.room.raceStartAt);this.room.finishOrder.push(p.id);p.state.finished=true;await this.save();
      this.broadcast({type:'race_finish',v:PROTOCOL_VERSION,raceId:this.room.raceId,playerId:p.id,finishPlace:p.finishPlace,finishTime:p.finishTime,room:this.publicRoom()});return;
    }
    if(m.type==='return_lobby'){
      ...
      this.room.status='lobby';this.room.raceId=null;this.room.raceStartAt=0;this.room.gridOrder=[];this.room.finishOrder=[];for(const x of this.room.players){x.ready=false;x.finishPlace=0;x.finishTime=0;x.state=null;x.lastSeq=-1;x.lastStateAt=0;x.serverLaps=0;x.lastLapAt=0;}
      await this.save();this.broadcast({type:'room_state',v:PROTOCOL_VERSION,serverTime:now(),room:this.publicRoom()});return;
    }
    if(m.type==='leave'){this.room.players=this.room.players.filter(x=>x.id!==p.id);if(this.room.hostId===p.id)this.room.hostId=this.room.players.find(x=>x.connected)?.id||this.room.players[0]?.id||null;if(!this.room.players.length){this.room=null;await this.ctx.storage.deleteAll();try{ws.close(1000,'leave');}catch{};return;}await this.save();this.broadcast({type:'room_state',v:PROTOCOL_VERSION,serverTime:now(),room:this.publicRoom()},ws);try{ws.close(1000,'leave');}catch{};return;}
  }
  ...
  async handleDisconnect(ws){
    ...
    // Ignore stale close from a socket already replaced by a reconnect.
  ...
  async alarm(){
    await this.load();if(!this.room)return;const t=now();const removed=this.room.players.filter(p=>!p.connected&&p.disconnectedUntil&&p.disconnectedUntil<=t);if(removed.length){this.room.players=this.room.players.filter(p=>!removed.includes(p));if(!this.room.players.some(p=>p.id===this.room.hostId)){this.room.hostId=this.room.players.find(p=>p.connected)?.id||this.room.players[0]?.id||null;}}
    if(!this.room.players.length||t>=this.room.createdAt+ROOM_TTL_MS||(!this.room.players.some(p=>p.connected)&&this.room.emptySince&&t>=this.room.emptySince+EMPTY_ROOM_TTL_MS)){this.room=null;await this.ctx.storage.deleteAll();return;}
    await this.save();this.broadcast({type:'room_state',v:PROTOCOL_VERSION,serverTime:t,room:this.publicRoom()});
  }
}
